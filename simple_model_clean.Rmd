...
beta_prior  <- rnorm(n_samples, -0.5, 0.5)
```
...
```{r stan-sampling, warning=FALSE, message=FALSE, results='hide'}
# ========= running stan ================
...
```{r stan-variational, message=FALSE, warning=FALSE, results='hide'}
grid_pred_pos <- 10
...
```{r calibration-mcmc-loop, warning=FALSE}
...
merged_data_exp %>% 
  ...
  geom_errorbar() +
  theme_minimal() +
  labs(title = "MCMC posterior predictive calibration (80% CI)",
     x = "Grid Position", y = "Gap to Winner (seconds)")

mean(merged_data_exp$Inside_CI)
```

```{r calibration-vi-loop}
all_vi_samples <- list()
...
for (i in 1:length(validation_obs)) {
  ...
  vi_results = mod_simple_exp$variational(
    seed = 1,
    data = train_test_dta,
    output_dir = "stan_out",
    refresh = 0,
  ...
  simple_exp_var_samples = as.vector(vi_results$draws("gap_pred"))
  all_vi_samples[[i]] <- as.vector(vi_results$draws("gap_pred"))
...
mcmc_trace(fit_simple_exp$draws("alpha")) + 
  theme_minimal() + 
  labs(title = "Trace plot: alpha (MCMC)")
        
mcmc_trace(fit_simple_exp$draws("beta")) + 
  theme_minimal() + 
  labs(title = "Trace plot: beta (MCMC)")

mcmc_trace(fit_exp.variational$draws("alpha")) + 
  theme_minimal() + 
  labs(title = "Trace plot: alpha (VI)")
mcmc_trace(fit_exp.variational$draws("beta")) + 
  theme_minimal() + 
  labs(title = "Trace plot: beta (VI)")
```
...
for (i in sample(1:nrow(simple_exp_draws_df), 50)) {
  rate_pred <- exp(simple_exp_draws_df$alpha[i] + 
                   simple_exp_draws_df$beta[i] * log(data$grid))
...
```
The black line shows the observed gap distribution while the red lines show 
50 datasets simulated from the posterior. The model captures the overall 
right-skewed shape but underestimates the spread in the middle range 
(30-100 seconds), suggesting the pooled Exponential model is an 
approximation. This motivates the hierarchical model which allows 
circuit-specific effects to better capture this variation.


```{r posterior-curves-over-data}
# model against data
...
# simulate_data: generates synthetic F1 gap data from the prior predictive
# distribution. Used to verify model correctness on known ground truth.
...
```{r debug-calibration-plot}
# check coverage on synthetic data
...
hist(initial_dist_df$forward_values.beta, col = "#a93226")
```

```{r exact-invariance-final}
# final distribution
final_dist_df = data.frame()
for (i in 1:100) {
  forward_values = simulate_data()
  dta = list(
    N = length(forward_values$grid),
...
hist(final_dist_df$beta, col = "#a93226")
```

```{r exact-invariance-ks}
ks.test(initial_dist_df$forward_values.beta, final_dist_df$beta)

hist(initial_dist_df$forward_values.beta, col = "#a93226",  border = "navy")
hist(final_dist_df$beta, col = "#a93226", border = "navy")
```

```{r compare-mcmc-vi-ci}
# ========= MCMC vs VI ================
